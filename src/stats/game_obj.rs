...
impl GameObject {
    ...
    pub fn try_create(id1: Identity, game1: TeamBoxScore, id2: Identity, game2: TeamBoxScore) -> Result<GameObject, Vec<CorrectionBuilder>> {
        let mut correction1 = CorrectionBuilder::new(id1.game_id, id1.season_id, None, id1.team_id, id1.team_abbr(), NBAStatKind::Team, id1.game_date);
        let mut correction2 = CorrectionBuilder::new(id2.game_id, id2.season_id, None, id2.team_id, id2.team_abbr(), NBAStatKind::Team, id2.game_date);
        ...
        let game = home_and_away(game1.clone(), game2.clone()).map(|(home,away)| {
        ...
        correction1.update_display(GameDisplay::new(matchup.clone(), id1.game_date, None, game1.team_name()));
        correction2.update_display(GameDisplay::new(matchup.clone(), id2.game_date, None, game2.team_name()));

        if correction1.correcting() || correction2.correcting() {
            Err(vec![correction1, correction2])
        } else {
            let (home, away) = game.expect("💀 if home_and_away errs it must be covered in the correcting branch.");
