...
impl EloTracker {
    ...
    fn update_ratings(&mut self, slip: &GameCard, box_score: &GameObject) {
        let home_rating =
            self.normalized_ratings_from_iter(dbg!(slip.home_roster()).into_iter().map(|x| *x));
        let away_rating =
            self.normalized_ratings_from_iter(dbg!(slip.away_roster()).into_iter().map(|x| *x));
        ...
        let game_ratings = GameRatings::new(&box_score.card(), &self.current_ratings);
...
inventory::submit!(Registration {
    ...
    factory: |args| {
        ...
        let step = args.get_one::<i64>("step").copied().unwrap_or(STEP);

        let initial_rating = args
            .get_one::<i64>("initial-rating")
            .copied()
        ...
        params.set_initial_rating(initial_rating);
...
mod test_elo_tracker {
    ...
    #[test]
    fn test_process_elo() {
        let mut tracker = EloTracker::params(EloParams::new(&Vector::from(vec![100., 1000., 0.0])));

        dbg!(tracker.initial_rating());
